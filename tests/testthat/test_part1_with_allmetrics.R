...
test_that("Part 1 can run with all metrics", {
  ...
  expect_that(round(mean(M$metashort$BFEN), digits = 4), equals(0.0473))
  expect_that(round(mean(M$metashort$LFENMO), digits = 4), equals(0.0451))
  expect_that(round(mean(M$metashort$HFENplus), digits = 4), equals(0.0933))
  expect_that(round(mean(M$metashort$MAD), digits = 4), equals(0.0075))
  expect_that(round(mean(M$metashort$ZCX), digits = 2), equals(15.4))
  expect_equal(sum(M$metashort$BrondCount_x), 17694)
  expect_equal(sum(M$metashort$BrondCount_y), 63927)
  expect_equal(sum(M$metashort$BrondCount_z), 1009963)

...
- In all .Rd files and when quotes are not surrounded by \` \` in .Rmd files precede each quote with a forward slash as in \\\" \\\".
...
I (Vincent van Hees) will run this command in the main branch prior to each new
GGIR release. If multiple contributors would be running this command in their own
development branche(s) we risk complex merge conflicts between the many html files
that are then auto-generated by this function.
...
For example, you may be able to provide feedback on or all of the following:
- Code functionality
- Code clarity

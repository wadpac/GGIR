detectTypeBouts = function(myfun, varnum_type, varnum, 
  ...
  for (class in valid_classes) {
    ...
    for (acc_th in myfun$tbout.th.acc) {
      ...
      for (boutdur in myfun$tbout.dur) {
        ...
        # 1. Create binary mask based on condition
        rr1 = matrix(0, length(varnum), 1)
        if (myfun$tbout.condition == "AND") {
          p = which(varnum * UnitReScale >= acc_th & varnum_type == class)
          rr1[p] = 1
        ...
        tboutname = paste0("ExtFunType_totdur_B", boutdur,
                           ...
                           "%_", class, "_", myfun$tbout.condition,
                           "_acc", acc_th)

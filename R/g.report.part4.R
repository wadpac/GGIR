g.report.part4 = function(datadir = c(), metadatadir = c(), loglocation = c(), f0 = c(), f1 = c(), storefolderstructure = TRUE,
  ...
  # set to TRUE)
  # function to remove na from dataset (for later use)
      ...
      for (dotwice in 1:2) {
        ...
        if (nrow(nightsummary) == 0) {
          ...
          if (dotwice == 1) {
            write.csv(nightsummary_round, file = paste(resultfolder, "/results/QC/part4_nightsummary_sleep_full.csv",
                                                 sep = ""), row.names = FALSE)
            write.csv(personSummary_round, file = paste(resultfolder, "/results/QC/part4_summary_sleep_full.csv",
                                                  sep = ""), row.names = FALSE)
          } else {
            write.csv(nightsummary_round, file = paste(resultfolder, "/results/part4_nightsummary_sleep_cleaned.csv",
                                                 sep = ""), row.names = FALSE)
            write.csv(personSummary_round, file = paste(resultfolder, "/results/part4_summary_sleep_cleaned.csv",
                                                  sep = ""), row.names = FALSE)
          }
        }
      }

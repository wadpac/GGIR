g.impute = function(M, I, params_cleaning = c(), desiredtz = "",
  ...
  pattern = paste("timestamp", externalDataColname, sep = "|")
  if (is.null(externalDataColname)) pattern = "timestamp"
  metrics2impute = grep(pattern, colnames(metashort), invert = TRUE)

  for (mi in metrics2impute) {# generate 'average' day for each variable
  ...
  metashort[,metrics2impute] = round(metashort[,metrics2impute], digits = n_decimal_places)

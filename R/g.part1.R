  ...
  main_part1 = function(i, params_metrics, params_rawdata,
    ...
    if (skip == 0) { #if skip = 1 then skip the analysis as you already processed this file
      ...
      if (!is.null(params_general[["recordingEndSleepHour"]])) {
        ...
        cat("\n")
        last_ts[1] = iso8601chartime2POSIX(x = tail(M$metalong$timestamp, n = 1), tz = params_general[["desiredtz"]])
        last_ts[2] = iso8601chartime2POSIX(x = tail(M$metashort$timestamp, n = 1), tz = params_general[["desiredtz"]])

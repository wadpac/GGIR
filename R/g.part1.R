g.part1 = function(datadir=c(),outputdir=c(),f0=1,f1=c(),windowsizes = c(5,900,3600),
        ...
        # check whether these are movisens files (data structure = 1 directory per participant)
        if (TRUE %in% grepl("acc.bin", fnames) & 
            TRUE %in% grepl("temp.bin", fnames) &
            TRUE %in% grepl("angularrate.bin", fnames)){     #then, it is movisens (detect now because folder structure is different)
        ...
        if (filelist == FALSE & is.mv) {
                fnamesfull = c(dir(datadir,recursive=TRUE,pattern="acc.bin", full.names = TRUE))

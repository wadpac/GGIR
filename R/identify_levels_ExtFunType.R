identify_levels_ExtFunType = function(ts, myfun, ws3) {
  ...
  if (!is.null(myfun) && length(myfun) > 0 &&
      "reporttype" %in% names(myfun) && myfun$reporttype == "type") {
    ...
    if (length(type_columns) > 0) {
      ...
      type_levels = sub(pattern = paste0("^ExtFunType_", myfun$colnames, "_"),
                        replacement = "", x = type_columns)
      type_levels = sub("_s$", "", type_levels)
      type_levels = tolower(type_levels)
      
      # Seconds assigned to each type in each epoch
      TOLEVELS = as.matrix(ts[, type_columns, drop = FALSE])
      colnames(TOLEVELS) = type_levels
      
      #---------------------------------------------
      # Default bout parameters
      if (!"tbout.dur" %in% names(myfun)) myfun[["tbout.dur"]] = c(1, 5, 10)
      if (!"tbout.criter" %in% names(myfun)) myfun[["tbout.criter"]] = 0.8
      if (!"tbout.order" %in% names(myfun)) myfun[["tbout.order"]] = sort(type_levels)
      ...
      type_order = c(type_order, setdiff(type_levels, type_order))
      ...
      for (type_level in type_order) {
        # level of interest in this iteration
        ...
        for (BL in 1:NBL) {
          ...
          p = which(ts[, type_col] > 0 &   # Epochs classified as this type
                      refe.type == 0)      # and not assigned to a higher-priority type
          rr1[p] = 1
          
          # identify
          out1 = g.getbout(x = rr1,
                           boutduration = boutduration[BL],
                           boutcriter = myfun[["tbout.criter"]],
                           ws3 = ws3)
          TLEVELS[[type_level]] = rbind(TLEVELS[[type_level]], out1)
          refe.type = refe.type + out1
          
          # Name the bout according to its duration
          if (BL == 1) {
            bout_name = paste0(type_level, "_bts_", myfun[["tbout.dur"]][BL])
          } else {
            bout_name = paste0(type_level, "_bts_", myfun[["tbout.dur"]][BL],
                               "_", myfun[["tbout.dur"]][BL - 1])
          }
  ...
  # ----------------------
  # Return
  invisible(list(TLEVELS = TLEVELS, TOLEVELS = TOLEVELS, Tnames = Tnames))
}
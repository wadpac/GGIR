g.part5_analyseSegment = function(indexlog, timeList, levelList,
  ...
  if (length(sse) >= 1) { #next
    ...
    if (!is.null(params_general[["externalDatadir"]])) {
      if (params_general[["externalDataColname"]] %in% colnames(ts)) {
        for (wii in c(0, 1, 2)) {
          if (wii == 0) wname = "_day"
          if (wii == 1) wname = "_spt"
          ...
          if (wii != 2) {
            ...
            varnum = ts[select, params_general[["externalDataColname"]]]
          } else {
            varnum = ts[sse, params_general[["externalDataColname"]]]
          }
          ...
          ds_names[fi] = paste0(params_general[["externalDataColname"]], wname, "_Nrecords")
          fi = fi + 1
          # MEAN
          dsummary[si, fi] = mean(varnum, na.rm = TRUE)
          ds_names[fi] = paste0(params_general[["externalDataColname"]], wname, "_mean")
          fi = fi + 1
          # MEDIAN
          dsummary[si, fi] = median(varnum, na.rm = TRUE)
          ds_names[fi] = paste0(params_general[["externalDataColname"]], wname, "_median")
          fi = fi + 1
          # SD
          dsummary[si, fi] = sd(varnum, na.rm = TRUE)
          ds_names[fi] = paste0(params_general[["externalDataColname"]], wname, "_sd")
          fi = fi + 1
          # CV
          dsummary[si, fi] = (sd(varnum, na.rm = TRUE) / mean(varnum, na.rm = TRUE)) * 100
          ds_names[fi] = paste0(params_general[["externalDataColname"]], wname, "_cv")
          fi = fi + 1
          # levels
          if (!is.null(params_247[["external_ilevels"]])) {
            ...
            for (rq52i in 1:length(rownames(q52))) {
              ds_names[fi] = paste0(params_general[["externalDataColname"]], wname,

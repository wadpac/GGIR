g.analyse =  function(I,C,M,IMP,qlevels=c(),qwindow=c(0,24),quantiletype = 7,L5M5window = c(0,24),M5L5res=10,
  ...
  nfeatures = 50+NVARS*(20+length(qlevels)+length(ilevels))
  if (length(qwindow) > 0) {
    ...
    for (di in 1:ndays) { #run through days
      ...
      if (tooshort == 0) {
        ...
        if (nvalidhours >= includedaycrit) {
          ...
          cat(paste0("anwi_t01 ",anwi_t0,anwi_t1))
          cat(" t3 ")
            ...
            for (mi in 2:ncol(metashort)) { #run through metrics (for features based on single metrics)
              ...
              if (mi == ENMOi | mi == LFENMOi | mi == BFENi | mi == ENi | mi == HFENi | mi == HFENplusi | mi == MADi) { #currently intensity/activity level features are based on metric ENMO, but by copy-pasting this to another metric this should work the same.
                ...
                if (anwi_index == 1) { # don't do this part for qwindow
                  ...
                  if (doilevels == TRUE) {
                    breaks = ilevels
                    q47 = cut((varnum[((t_TWDI[1]*60*(60/ws3))+1):(t_TWDI[2]*60*(60/ws3))]*1000),breaks,right=FALSE)
                    q47 = table(q47)

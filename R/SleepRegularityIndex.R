SleepRegularityIndex = function(data = c(), epochsize = c(), desiredtz= c()) {
  ...
  for (j in 1:M) { # epoch
    for (i in 1:(N-1)) { # day
      SummedValue = SummedValue + ifelse(test = (sleepstate_byday[j,i] == sleepstate_byday[j,i+1]), yes = 1, no = 0)
    }
  }
  SRI = round(-100 + (200/(M*(N-1))) * SummedValue, digits = 3)
  return(SRI)
}
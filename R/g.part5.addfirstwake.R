g.part5.addfirstwake =function(ts, summarysleep_tmp2, nightsi, sleeplog, ID, 
                               Nepochsinhour, Nts, HDCZA_end, ws3new) {
  ...
  if (is.na(HDCZA_end[1]) == TRUE) {
    HDCZA_end = HDCZA_end[which(is.na(HDCZA_end) == FALSE)]
  }
    ...
    if (length(sleeplog) > 0) {
      ...
      if (length(wake_night1) != 0 & length(onset_night1) != 0) {
        if (wake_night1 != "" & onset_night1 != "") {
          ...
          wake_night1_index = nightsi[1] + round((HDCZA_end[1]-24) * Nepochsinhour)
        }
      } else { # use HDCZA algorithm as plan B
        wake_night1_index = nightsi[1] + round((HDCZA_end[1]-24) * Nepochsinhour)
      }
    } else if (length(HDCZA_end) > 0 & length(sleeplog) == 0) {
      # use HDCZA algortihm for waking up after first night
      # if there was no sleep log
      if (is.na(HDCZA_end[1]) == FALSE) {
        if (HDCZA_end[1] != 0) {
          wake_night1_index = round((HDCZA_end[1]-24)* Nepochsinhour)
        }
      }
    }

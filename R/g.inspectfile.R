g.inspectfile = function(datafile, desiredtz = c(), ...) {
        ...
        getbrand = function(filename=c(),datafile=c()) {
                ...
                if (TRUE %in% grepl("acc.bin", dir(datafile)) & 
                    TRUE %in% grepl("temp.bin", dir(datafile)) &
                    TRUE %in% grepl("angularrate.bin", dir(datafile))){     #then, it is movisens
                        dformat = 1 #1 = binary
                        mon = 5 # Movisens
                        is.mv = TRUE
                } else{
                ...
                if (dformat == 1) { # .bin
                        if (is.mv == TRUE){
                                sf = 64   # Sample rate for movisens is fixed at 64 Hz
                        } else if("GENEAread" %in% rownames(installed.packages()) == FALSE) {
                        ...
                        } else if (length(isitageneactive) > 0){
                                if (length(as.matrix(isitageneactive)) >= 1 & length(isitagenea) == 0) {

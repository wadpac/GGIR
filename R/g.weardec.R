g.weardec = function(M,wearthreshold,ws2) {
  ...
  r2 = c(0, r2, 0)
  r3 = c(0, r3, 0)
  ch1 = which(diff(r1) == 1) + 1 # indices of start wear
  ch2 = which(diff(r1) == -1) + 1 # indiced of start non-wear)
  if (length(ch1) > 1) { # there are at least 2 non-wear periods
    ...
    for (weari in 1:(length(ch1) - 1)) {
      ...
      if (ch1[weari] > (length(timeline) - (24/(ws2/3600)))) { 
        # for last 24 hours of the data
        if (wear[weari,1] < (3/(ws2/3600)) & wear[weari,3] > (1/(ws2/3600))) {
  ...
  if (length(ch1) > 0) {
    ...
    if (ch2[length(ch2)] > (length(r3) - (3 / (ws2 / 3600))) &
        ch2[length(ch2)] != length(r3)) {
  ...
  for (i in 1:2) { #for 2 additional iterative phases
    ...
    r1b = c(0, r1b, 0)
    r3b = c(0, r3, 0)
    ch1 = which(diff(r1b) == 1) + 1
    ch2 = which(diff(r1b) == -1) + 1
    if (length(ch1) > 1) {
      #there are at least 2 non-wear periods
      wear = matrix(0, (length(ch1) - 1), 3) # characteristics of all wear period in between non-wear periods
      for (weari in 1:(length(ch1) - 1)) {
        wear[weari, 1] = abs(ch1[weari + 1] - ch2[weari]) # length of wear
        wear[weari, 2] = abs(ch2[weari + 1] - ch1[weari + 1]) # length of non-wear after it
        ...
        if (ch1[weari] > (length(timeline) - (24 * 4))) {
          #for last 24 hours of the data
          if (wear[weari, 1] < (3 / (ws2 / 3600)) &

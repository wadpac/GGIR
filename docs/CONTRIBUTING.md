...
Conduct](https://wadpac.github.io/GGIR/CODE_OF_CONDUCT.md).
...
1.  use the search functionality
    [here](https://github.com/wadpac/GGIR/discussions) to see if someone
...
To ease copy-pasting code from documentation to R, use the right type of
quotes " " when providing examples of character parameter values.
...
argument in functions and . The pull request template has a form that
will help you check all these points.
...
use [the pkgdown configuration
file](https://wadpac.github.io/GGIR/_pkgdown.yml) that can be found in
the repositories root directory, as well as with the chapter vignettes
discussed above.

General comment: As contributor please **NEVER** run the command
[`pkgdown::build_site()`](https://pkgdown.r-lib.org/reference/build_site.html).
I (Vincent van Hees) will run this command in the main branch prior to
each new GGIR release. If multiple contributors would be running this
command in their own development branche(s) we risk complex merge
conflicts between the many html files that are then auto-generated by
this function.
...
2.  Open the [\_pkgdown.yml](https://wadpac.github.io/GGIR/_pkgdown.yml)
    file and fill up the name and reference of the new chapter under
    menu. Make sure to follow the coding and structure of the rest of
    chapters.
...
1.  Remove the lines corresponding to the chapter in the
    [\_pkgdown.yml](https://wadpac.github.io/GGIR/_pkgdown.yml) file in
    line 42 onwards.
...
Chapter names are defined twice, in the
[\_pkgdown.yml](https://wadpac.github.io/GGIR/_pkgdown.yml) file and in
the vignette file itself. You need to make sure both titles match as the
first will be used in the drop-down list in the github.io website and
the other in the specific page for the chapter.
...
GGIR is primarily maintained by one person (Vincent) and depends on
pro-active community efforts to help screen from problems. Therefore, we
welcome voluntary efforts to peer-review [open pull requests
...
repository collaborator, and will assign you as a reviewer.

## New release

GGIR follows the [release cycle process described in this
document](https://wadpac.github.io/GGIR/RELEASE_CYCLE.md).

...
Various GGIR parameters expect us to provide a date or time format. R
uses codes to define elements of a date or time format.
...
The table below shows examples of how to define time or data format
...
- `rmc.format.time = "%d/%m/%Y %H:%M:%S"` when we see
  `"20/03/2025 15:05:02"` in a csv file with raw data.

- `extEpochData_timeformat = "%d-%m-%Y %H:%M:%S"` when we see
  `"12-03-2025 15:05:02"` in an externally derived epoch data file such
  as the Philips health band .xlsx data files.

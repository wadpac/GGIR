---
title: "Luminous (light) exposure analysis"
output:
   html_document:
    toc : true
...
Light exposure, scientifically referred to as luminous exposure, is gaining importance. Many accelerometers as used in research have a light sensor. In this vignette we discuss what GGIR complemented by LightLogR can do with these data.
...
R package [LightLogR](https://tscnlab.github.io/LightLogR/index.html) offers a much more in-dept analysis of lumnious exposure and has been developed by experts on luminuous exposure. In this vignette we will document how GGIR epoch level output can be used as input for LightLogR.
...
data %>% 
  LightLogR::filter_Date(length = "1 week", full.day = TRUE) %>% #restrict data length
...
data %>% 
  ...
  #calculating metrics
  dplyr::summarize(
  TAT250 = duration_above_threshold(lightmean, Datetime, threshold = 250),
  .groups = "drop_last"
  ) %>% 
  #calculating the average and standard deviation of the TAT250
  dplyr::summarize(TAT250_average = mean(TAT250) %>% 

...
-   **(Optionally, not by default) Per segment of a recording**. For example, when a person is tracked for multiple weeks and you want to analyse the segment before before and after an intervention seperately. For details on how GGIR can split a recording into multiple new recordings, see documentation on GGIR parameters `recording_split_times`, `recording_split_overlap`, and `recording_split_timeformat` and documentation on how to use GGIR parameters as explained further down.

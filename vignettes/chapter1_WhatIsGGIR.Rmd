...
-   **(Optionally, not by default) Per sequence of recordings with matching participant IDs**. For example, when a person is tracked first with one accelerometer and then the accelerometer is replaced by a different accelerometers GGIR is able to append those recordings and treat them as a single recording. For details on how to use this, see documentation on GGIR parameter `maxRecordingInterval` and documentation on how to use GGIR parameters as explained further down.
-   **(Optionally, not by default) Per segment of a recording**. For example, when a person is tracked for multiple weeks and you want to analyse segments of the recordings separately. For example, before and after an intervention. For details on how GGIR can split a recording into multiple new recordings, see documentation on GGIR parameters `recording_split_times`, `recording_split_overlap`, and `recording_split_timeformat` and documentation on how to use GGIR parameters as explained further down.
...
We have structured the chapters in line with the GGIR training course we have been organising in recent years. Further, we also wanted to provide a good level of documentation for those who do not follow the course or want to refresh their understanding of GGIR.

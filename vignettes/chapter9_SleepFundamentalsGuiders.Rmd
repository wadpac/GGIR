...
- If a column name includes the word "impute" then this column is treated as log of your own diary data imputation (outside GGIR) for the night ending on the current day. The value will be shown in the visualreport and is stored in the time series by GGIR part 5. The imputation codes we used for testing have the format C0000 to indicate no imputation for inbed, sleeponset, wakeup and outbed, respectively. If one of the numbers would be 1 then that would reflect imputation.
...
In our experience data-driven guiders such as HDCZA can struggle with the following scenarios:
...
To address this an algorithm has been proposed. The algorithm is currently being optimised and this part of the documentation will be updated once the optimisation has been finalised.
...
1. Apply default HDCZA to each night, but retain all candidate sleep blocks whereas normally we would only retain the longest block and use that as guider.
2. Identify min-max window running from the minimum sleep onset to maximum wakeup time from all main HDCZA across all nights in one recording.
3. If there are at least 3 (parameter: `guider_cor_meme_min_dys`) valid nights:
    a. Identify median-median window running from the median sleep onset until the median wakeup time from all main HDCZA across all nights in one recording.
    b. Loop over valid nights and select an alternative main HDCZA if the default main HDCZA is >90% outside the median-median window (parameter: `guider_cor_meme_frac_out`) and there is a secondary HDCZA window that:
    ...
    c. Identify min-max window (step 1) again but now based on the corrected main HDCZA from step 2b.
4. Loop over nights and expand the main HDCZA towards neighbouring HDCZA windows  if those secondary windows:
    ...
    d. Are not separated from the main HDCZA window by more than 3 hours (parameter: `guider_cor_maxgap_hrs`).

If you would want to skip step 3 set parameter `guider_cor_meme_min_dys = Inf`.

...
The algorithm for auto-detection of sleep as described in the non-public document by Cambridge Neurotechnologies: Information bulletin no.3 sleep algorithms. The algorithm uses a combination of count and marker data to identify the sleep periods in the day, where GGIR for now only retains the longest identified sleep period.

I have implemented it as an attempt to explore its value.


### Guider: HLRB (EXPERIMENTAL)

HLRB is a heuristic algorithm to detect the Longest Rest Bout. It was designed to offer a guider option for accelerometer that only store count data and by that cannot work with the HDCZA or HorAngle guiders.

The algorithm is composed of the following steps:

1. Filter detected sustained inactivity bouts with a rounded two hour rolling average.
2. Replace all resulting non-rest periods by rest that are surrounded on both sides by rest if their (non-rest) length is shorter than 1 hour.
...
5. From these marker events, ignore all marker event pairs that both span less than 3 hours and involve at least one imputed marker button press, unless these represent all marker button pairs available.
6. Ignore all marker event pairs where the percentage of sustained inactivity bouts inside the window is larger than outside the window.

...
1. GGIR works with the following accelerometer brands and formats:
    ...
    - Any other accelerometer brand that generates csv output, see function documentation for function read.myacc.csv and the documentation for argument rmc.noise.
...
Studies done with ActiGraph devices when configured with 'idle sleep mode' turned on, will have zero-strings in all three axes during periods of no movement. Studies do not clarify how these zeros strings are accounted for. The insertion of zero strings is problematic as raw data accelerometers should always measure the gravitational component when not moving. This problematic decision by the ActiGraph company directly impacts metrics that rely on the presence of a gravitational component such as ENMO, EN, ENMOAa, and SVMgs. Further, also other metrics may be affected as the sudden disappearance of gravitational acceleration will result in a spike at the start and end of the no movement period. More generally speaking, we advise ActiGraph users to disable the 'idle sleep mode' as it harms the transparancy and reproducibility of the research as the software used for this is not open source, it reduces comparability with other accelerometer brands which is bad for research harmonization, and it challenges the accurate assessment of sleep and sedentary behaviour. Further, data collected with 'idle sleep mode' should not be refered to as raw data accelerometry, as the data collection process involves proprietary pre-processing steps, which is against the philosophy of raw data collection.

Several studies that aimed to independently evaluate cut-point methods seem to be failing at recognising these challenges. Therefore, it is best to interpret cut-points with caution that are potentially not more than crude boundary between intensity levels. Future methodological studies around cut-points are advised to account for accelerometer calibration error and the problematic time gaps in for example the ActiGraph when configured with 'idle sleep mode'.

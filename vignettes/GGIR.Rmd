...
When you use GGIR in publications do not forget to cite it properly to make your analysis reproducible for others and to give credit to it's developers. See paragraph on [Citing GGIR](#CitingGGIR) for details.
...
Furthermore, studies that aimed to independently evaluate cut-point methods also seem to be failing in recognising this challenge. Therefore, it seems best to interpret cut-points as a crude boundary between intensity levels that should be interpreted with caution. Future methodological studies around cut-points are advised to account for accelerometer calibration error.
...
- [Dibben et al. 2020](https://bmcsportsscimedrehabil.biomedcentral.com/articles/10.1186/s13102-020-00196-7) does not report sample frequency by which the proposed cut-points cannot be replias we need the original sample frequency for that.
...
- **Sadeh1994:** The algorithm proposed by Sadeh et al. [link](https://doi.org/10.1093/sleep/17.3.201) was originally designed for epoch level count values generated by the AMA-32 device by Ambulatory Monitoring Inc. These counts represent
the number of zero crossing per epoch window. GGIR attempts to replicate the calculation of the count for modern raw acceleration signals based on available description of the original approach and knowledge about how modern acceleration sensors differ from acceleration sensors in the 1980s and 1990s. However, it should be noted that the documentation on the original sensor is incomplete, by which correct reproduction cannot be guaranteed. Missing information includes: (1) Sadeh specified that calculations were done on data from the Y-axis but the direction of the Y-axis was not clarified. It is, therefore, unclear whether the Y-axis at the time corresponds to the Y-axis of modern sensors, (2) Properties of the frequency filter are missing like the filter order and more generally it is unclear how to simulate original acceleration sensor behaviour with modern sensor data, (3) Sensitivity of the sensor, we are now guessing that it was 0.01 g but without direct proof. In modern commercial actigraphy sensors the counts are derived with closed-source software and/or hardware with the claim that the Sadeh algorithm was used. It is impossible to verify their claim because the original devices are not used anymore and because of the closed nature of the commercial technology. To use the GGIR implementation of the zero-crossing counts and Sadeh algorithm, specify argument `HASIB.algo = “Sadeh1994"` and argument `Sadeh_axis = "Y"` to indicate that the algorithm should use the Y-axis of the sensor.
- **Galland2012:** The count-scaled algorithm proposed by Galland et al. [link](http://dx.doi.org/10.1016/j.sleep.2012.01.018), uses counts generated by closed-source commercial technology resulting in effectively the same challenge as with the Sadeh algorithm. In order to use our implementation of the Galland2012 algorithm we need to specify argument `HASIB.algo = “Galland2012"`. Further, set `Sadeh_axis = "Y"` to specify that the algorithm should use the Y-axis.
...
5.  Studies who have criticised ENMO consistently failed to apply auto-calibration, or attempted to apply auto-calibration in a lab setting, ignoring the fact that the auto-calibration is not designed for short lasting lab settings. It needs free-living data to work.

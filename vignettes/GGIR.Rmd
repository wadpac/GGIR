...
### Tailoring GGIR to study protocol

Usual study protocols to collect multiday accelerometer raw data last one
week, either asking participants to wear accelerometers allday long (i.e., 24/7)
or only the time they are awake. GGIR includes a number of arguments to 
facilitate the processing of files collected with different study protocols. 
For example:
...
-   `strategy` - informs GGIR how to consider the design of the
    ...
    -   If `strategy` is set to value 2, then GGIR will use the data
        available between the first and the last midnight.
    -   If `strategy` is set to value 3, then check out arguments
        `ndayswindow`.
    -   If `strategy` is set to value 4, then GGIR will use the data
        from the first midnight until the very end of the file.
-   `maxdur` - maximum number of days you expect in a data file based on
    the study protocol.
-   `AskedToWear247` - whether participants were instructed to wear the devices
    during awake and sleep time or not. If `AskedToWear247 == FALSE`, then check
    out arguments `part2ImpWithZero` and `part2ExcludeNonwear`.
-   `part2ImpWithZero` - If `AskedToWear247 == FALSE`, this will be automatically set to 
    `TRUE` by GGIR. Then, the acceleration metric of interest during nonwear time 
    will be imputed by 0 to avoid bias from the imputation of long nonwear periods.
    This only applies when `do.imp == TRUE` (default configuration).
-   `part2ExcludeNonwear` - If `AskedToWear247 == FALSE`, this will be automatically set to 
    `TRUE` by GGIR. Then, the detected nonwear time will be excluded from the 
    analysis to avoid the overestimation of activities characterized by low
    acceleration values.
-   Other parameters - Please, see the [configuration parameters vignette](https://cran.r-project.org/web/packages/GGIR/vignettes/GGIRParameters.html) 
    for more information on parameters that facilitate handling with the study protocol
    (e.g., `max_calendar_days`, `excludefirstlast`, `excludefirst.part4`, `excludelast.part4`, `excludefirstlast.part5`).

...
Accelerometer non-wear time is detected on the basis of the standard
deviation and the value range of the raw data from each accelerometer
axis. A block is classified as non-wear time if the standard
deviation of the accelerations is less than 13.0 m*g*
($1 mg = 0.00981 m·s^−2$) and the range of accelerations (i.e., maximum value *minus* minimum value) 
is less than 50 m*g*, for at least two out of the three accelerometer axes.

The block to test these conditions can be defined by the user via the argument 
`windowsizes`, where the second (mediumsize window, default = 15 min) and third value 
(longsize window, default = 60 min) are used for the non-wear classification based
on the selected `nonwear_approach`. At the moment there are two approaches to detect
...
The 2013 algorithm is a revision of the algorithm first described in the [2011 PLoSONE publication](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0022922).
This non-wear detection approach is done per mediumsize window (default = 15 minutes) and based on
the characteristics of the longsize window (default = 60 minutes) centred at the mediumsize window 
to be flagged. 

The modification of the algorithm in 2013 included:

-   The longsize window passed from 30 minutes (original, 2011) to 60 minutes (current default, 2013)
to decrease the chance of accidentally detecting short sedentary periods as non-wear time.
-   The windows passed from non-overlapping to overlapping (15 minute steps, window
overlap of 45 minutes), which was done to improve the accuracy of detecting the boundaries of non-wear time.

**nonwear_approach = "2023"**
The 2023 version of the algorithm proposes marking as non-wear the longsize window (default = 60 minutes) 
being tested, instead of the mediumsize window in the middle (default = 15 minutes). 
Instead of centering the longsize window in the middle of the mediumsize window, the 2023 method aligns 
the longsizewindow with its left edge to the left edge of the mediumsize window.

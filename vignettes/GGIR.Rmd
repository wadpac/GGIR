...
GGIR uses by default the sleep log if available, if the sleep log is not available it falls back on the HDCZA algorithm, if this is not available it will falls back on the L5+/-12 definition, and if this is not available it will use the setwindow. The user can specify the priority with argument def.noc.sleep. So, when we refer to guider then we refer to one of these four methods.

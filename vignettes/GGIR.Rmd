...
1. GGIR works with the following accelerometer brands and formats:
    ...
    - [ActiGraph](https://actigraphcorp.com/) .csv and .gt3x. Note for Actigraph users:
...
In this paragraph we will guide you through the main arguments relevant for 99% of research. First of all, it is important to understand that GGIR is structured in two ways. It has a computational structure of five parts and that `g.shell.GGIR` controls each of these parts:
...
Secondly, the function arguments or input parameters used by GGIR are structured thematically independently of the five parts:
...
- params_247: parameters relatd to 24/7 behaviours that do not fall into the typical sleep or physical activity research category.
...
Documentation for this parameter objects can be found in the ([GGIR pdf manual](https://CRAN.R-project.org/package=GGIR/GGIR.pdf)). All of these arguments are also accepted by `g.shell.GGIR`, because `g.shell.GGIR` is a shell around those functions. However, the `params_` objects can not be provided as input to
`g.shell.GGIR`.

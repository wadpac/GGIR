...
- Metrics related to Gini, CoV, or power law exponent alpha are only calculated if the standard deviation in fragment duration is not zero, and if there are at least 10 fragments (5 inactive and 5 active). If this condition is not met the metric value will be set to missing. To keep an overview of how many recording days met this criteria, GGIR part 5 stores variable Nvaliddays_AL10F at person level (Nvaliddays At Least 10 Fragments).

...
Further, the IV algorithm by van Someren has been modified in GGIR to ignore transitions between the epoch at the end of each day and the epoch at the start of the next day, because technically those would relate to intradaily variability.

#### IV and IS - Cosinor analysis compatible

IS is sometimes used as a measure of behavioural robustness in combination with Cosinor analysis. However, to make a direct comparison between the two it is important that IS is calculated from the same time series. Therefore, when the user specifies `cosinor = TRUE` then IV and IS are calculated twice. Once as part of the default IV and IS analysis (described above), and once as part of the Cosinor analysis.

Here, IV and IS are calculated from the same log transformed time series as used for the Cosinor analysis. The IV and IS algorithm is applied with `IVIS.activity.metric = 2` and `IVIS_acc_threshold = log(20 + 1)` to make the binary distinction between active and inactive, and `IVIS_per_daypair = TRUE`. The setting `IVIS_per_daypair` was specifically designed for this context to handle the potentially missing values in the time series used for cosinor analysis. Applying the default IVIS analysis would not be able to handle the missing values and would result in a loss of information if all non-matching epochs were excluded. Instead, IV and IS are now calculated as follows:

1. Calculate both IS and IV per day pair based on matching valid epochs only, and keep a log per day pair of the number of valid epoch pairs.
...
You can find these new IV and IS estimates as output variables `cosinorIV` and `cosinorIS`.

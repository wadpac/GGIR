...
To aid research in exploring count type algorithms, we also implemented the BrondCounts as proposed by Br&#248;nd and Brondeel and available via R package activityCounts. To extract this metric in addition to the zero crossing count, specify `do.brondcounts = TRUE` which is used in GGIR part 1 and uses R package activityCounts in the background. As a result, sleep estimates for Sadeh or Galland will be derived based on both the zero crossing algorithm and the brond counts algorithms. 

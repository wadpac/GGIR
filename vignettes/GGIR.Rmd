...
| `IV`  | Intradaily Variability (IV) after Eus J. W. Van Someren (Chronobiology International. 1999. Volume 16, issue 4)      |
| `IS`  | Interdaily Stability (IS) after Eus J. W. Van Someren (Chronobiology International. 1999. Volume 16, issue 4) |
...
The IV and IS implementation in GGIR was revised in 2024, where old arguments
such as IVIS.activity.metric were deprecated. The revised calculation works as follows:

Accelerometer metric ENMO is collapsed into a binary score of inactive or active
by using parameter `threshold.lig`, invalid time points (nonwear) are set to NA,
next epochs are averaged per hour, which are then used as input to the algorithm
as original described.
...
segment of the recording with the only difference that the values are not log
transformed for IS and IV. Therefore, IV and IS are calculated as part of the Cosinor 
analysis using the same segment of the time series.

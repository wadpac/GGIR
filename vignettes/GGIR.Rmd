...
- Dr. Matthew R Patterson contributed to updating visualizations and bug fixes
...
- `viewingwindow` and `visualreport` - to create a visual report, this only works when all five parts of GGIR have succesfully run. If `visualreport` is set to 0, then no plots will be created. If `visualreport` is set to 1, the original plot will be created. If `visualreport` is set to 2, the new plot that contains all sleep and activity annotations will be created. If `visualreport` is set to 1 and 2 (c(1,2)), then both plots will be created.

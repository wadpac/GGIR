...
In some recordings some part of sleep is incorrectly detected as non-wear. The acceleration signal is entirely
without movement for at least an hour. We could address this by increasing the value of the non-wear detection window via the third value of parameter `windowsizes`, which by default is 3600 (seconds). However, this makes the algorithm insensitive to nonwear shorter than this length.

As a possible solution GGIR offers the option to filte, meaning: ignore, any detected non-wear during the night that lasts less than the number of hours specified via parameter `nonwearFiltermaxHours`. The window over which this step is applied is set via parameter `nonwearFilterWindow`.

...
The elegance of using a standard deviation calculated from each raw signal over a long time window such as 60 minutes is that it is sensitive to even the slightest change in sensor orientation, even if the orientation change itself may be brief and slow. The reason for this is that the orientation change causes a change in the gravitational component of the acceleration signal, which means a change in offset, which has a different impact on the standard deviation. Only when the accelerometer remains untouched and unmoved for the entire 60 minutes the standard deviation represents the noise in the acceleration signal. Additionally, the standard deviation may also pick movement-related acceleration, but human movement is not possible without any form of orientation change and by that it primarily is the sensitivity to orientation change that defines the non-wear detection.

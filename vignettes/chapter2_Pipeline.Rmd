...
Inside the output folder GGIR will create two subfolders: `meta` and `results` as discussed earlier in this chapter. Inside `results` you will find folder named `QC` (Quality Checks). The name QC (Quality Checks) is possibly somewhat confusing. Data quality checks are best started with the files stored in the results folder, where the files in the QC subfolder offer complementary information to help with the quality check. For a full GGIR analyses we expect at least the following output files:

| File path                                                | Resolution                                                       | GGIR part |
|--------------------------------------|-----------------|-----------------|
| results/part2_summary.csv                                | one row per recording                                            | 2         |
| results/part2_daysummary.csv                             | one row per day                                                  | 2         |
| results/part4_summary_sleep_cleaned.csv                  | one row per recording                                            | 4         |
| results/part4_nightsummary_sleep_cleaned.csv             | one row per night                                                | 4         |
| results/visualisation_sleep.pdf                          | 40 recordings per page                                           | 4         |
| results/part5_daysummary_WW_L40M100V400_T5A5.csv         | one row per day                                                  | 5         |
| results/part5_personsummary_WW_L40M100V400_T5A5.csv      | one row per recording                                            | 5         |
| results/QC/data_quality_report.csv                       | one row per recording                                            | 5         |
| results/QC/part4_summary_sleep_full.csv                  | one row per recording                                            | 4         |
| results/QC/part4_nightsummary_sleep_full.csv             | one row per night                                                | 4         |
| results/QC/part5_daysummary_full_WW_L40M100V400_T5A5.csv | one row per day                                                  | 5         |
| results/QC/plots_to_check_data_quality_1.pdf             | one recording per page                                           | 2         |
| meta/ms5.outraw/40_100_400                               | one file with time series per recording                          |           |
| meta/ms5.outraw/ behavioralcodes2023-11-06.csv           | dictionary for behavioural codes as used in the time series file |           |

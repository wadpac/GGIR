...
One practical value of the configuration file is that you can share it with others such that they can replicate your analyses via GGIR parameter `configfile`. The code below shows an example where `"D:/myconfigfiles/config.csv"` is used as configuration file. Note that parameters `datadir` and `outputdir` discussed below always need to be specified directly and are not part of the configuration file.

...
GGIR stores all the parameter values in a csv-file named config.csv. The file is stored after each run of GGIR in the root of the output folder, overwriting any existing config.csv file. So, if you would like to add annotations in the file, e.g. in the fourth column, then you will need to store it somewhere outside the output folder and specify the path to that file with parameter `configfile`. The code below shows an example of this, where `"D:/myconfigfiles/config.csv"` is used every time the script is run and can include your comments in the fourth column. Note that parameters `datadir` and `outputdir`discussed below always need to be specified directly and are not part of the configfile.
...
The practical value of this is that it eases the replication of the analysis, because instead of having to share your R script with colleagues, sharing your config.csv file will be sufficient to replicate the analysis. Please make sure you have the same GGIR and R version installed when using this for reproducibility. See [here](https://wadpac.github.io/GGIR/articles/chapter0_Installation.html) for guidance on how to install older package versions.

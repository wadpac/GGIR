...
Algorithms (e.g. machine learning models) are typically developed for a specific data format and do not necessarily include the necessary data cleaning steps involved in real life 'out of the lab' experiments of multiple days of data per participant. As a result, it is not easy to pilot or fully implement a new algorithm those datasets. Code will have to be written to read, clean, and aggregate the data in an efficient way.

GGIR now allows you to apply your algorithm written in R or Python to raw accelerometer data, while taking care of all those other processing steps. 
...
Do not forget to update the filepath to the `calculateCounts.R` file.
...
Please see to GGIR [\underline{vignette}](https://cran.r-project.org/package=GGIR/vignettes/GGIR.html) on what other arguments can be provided to the g.shell.GGIR function.

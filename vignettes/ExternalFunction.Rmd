...
**Challenge**
If you are developing an algorithm for processing raw accelerometer data you may be interested to pilot it on a dataset with multi-day recordings. For example, the algorithm could be an activity type classifier developed with lab experiment data and you may be curious what percentage of the week it classifies as walking an epidemiology study. However, to do it well you would have to build code to read, clean and aggregate the data in an efficient way. Developing such code takes a lot of time.
...
- `aggfunction` The function used for aggregation if the data needs to be aggregated to match the short epoch size of the g.part1 output (5 seconds), see description outputres.
...
Please see to GGIR [\underline{vignette}](https://cran.r-project.org/package=GGIR/vignettes/GGIR.html) on what other arguments can be provided to the g.shell.GGIR.
...
A specification of the Python environment to use, this can also be a conda environment or docker container (see documentation R package [\underline{reticulate}](https://rstudio.github.io/reticulate/) for details on that). Make sure that that this Python environment has all the required dependencies.

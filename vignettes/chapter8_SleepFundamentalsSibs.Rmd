...
The algorithm takes as input epoch-by-epoch count values. Given that multiple manufacturers have used the sleep scoring algortihm by Oakley we have to assume that the count values are calculated identically between their products. 

From what I have been able to retrieve from product documentation (source The MotionWatch User guide: Issue 1.4.20b, page 30) the count is calculated on-board the device in four stages:
...
The algorithm itself has been communicated by the manufacturers via product information documents. 
...
To use the GGIR implementation of this algortihm, specify parameter `HASIB.algo = "Oakley1997"`.

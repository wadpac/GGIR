...
If you are concerned that sleep estimates are not reliable without being guided by sleeplog and
sufficient valid data in both the preceding and following night, then use parameter `require_good_neighbours_part5 = TRUE`. This forces the GGIR part 5 csv output to only keep day window that are both preceded and succeeded by an SPT window that is guided by sleeplog and have sufficient valid data.

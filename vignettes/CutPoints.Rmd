...
However, GGIR also facilitates some metrics whose value is not expressed in 
gravitational units to facilitate comparability with historical methods. 
For example, the count metric described by Neishabouri (see GGIR argument
`do.neishabouricounts = TRUE`) to reflect the counts calculated by the ActiLife 
software by ActiGraph. Cut-points for ActiGraph counts have been recurrently 
proposed in the literature, for example, see this [systematic review](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6231536/) with cut-points 
stratified by age group. Note that cut-points for ActiGraph counts proposed before
the introduction of raw data collected (2009) are most likely device hardware-based 
calculation which may not align with ActiGraph software-based counts (Actilife) 
that Neishabouri described. As a result, older cut-points should may need to be 
used with extra caution.

The cut-points you find in the literature for ActiGraph counts cannot be applied 
to Neishabouri counts directly because both are epoch length specific. The cut-point 
from the literature needs to be correct by a conversation factor. The conversion 
factor is calculated as the epoch length in the new study (e.g. 5 seconds) divided 
by the epoch length original study (e.g. 60 seconds). Note that a no correction 
for differences in sampling rate is needed because Neishabouri counts already account 
for this via downsampling.

If we would want to use cut-point 100 counts per minute from the literature, the 
GGIR function call would look like this:

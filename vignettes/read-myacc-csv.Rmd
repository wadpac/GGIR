...
As for the rest of GGIR functions, `read.myacc.csv` is intended to be used within `g.shell.GGIR`. All the arguments of the `read.myacc.csv` can be easily recognized as the all start by "rmc". The g.shell.GGIR function checks whether the argument rmc.firstrow.acc is provided by the user; in such case, GGIR will derive the reading of the data to the `read.myacc.csv`.
...
We could read this file using the `read.myacc.csv` function in GGIR as follows.
...
If the `rmc.firstrow.acc` argument is defined within the `g.shell.GGIR` function, then the data will be read through `read.myacc.csv`. GGIR needs the user to specify in which row starts the accelerometer data within the csv, so this argument must be always explicitly specified by the user. Thus, a call to the `g.shell.GGIR` including the rmc arguments would look as follows (note that the `rmc.file` argument does not need to be specified as the datadir already contains such information).
